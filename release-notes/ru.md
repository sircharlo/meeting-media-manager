...
- 🛠️ **Улучшенные заголовки разделов**: Новая система меню с выбором цвета, перемещением вверх/вниз, опциями повтора и функционалом удаления.
- ✨ **Встроенное редактирование заголовков**: Редактирование медиа-элементов непосредственно в интерфейсе без открытия отдельных диалоговых окон.
- 🛠️ **Улучшенная навигация**: Улучшены сочетания клавиш с функцией прокрутки к выбранному элементу и расширенная навигация по медиа.
...
- 🛠️ **Лучшая обработка MIME-типов**: Улучшена поддержка MIME-типов для лучшей совместимости с медиа-файлами.
- 🛠️ **Расширенная навигационная панель**: Улучшена обработка состояний мини-панели и добавлена подсказка для лучшей навигации.
- 🛠️ **Совместимость с Linux**: Принудительное использование GTK 3 в Linux для предотвращения проблем с интерфейсом и запуском.
...
- 🔍 **Сброс масштаба с помощью `Ctrl` + `Прокрутка`**: Включен быстрый сброс масштаба с помощью жеста Ctrl + прокрутка для облегчения навигации.
- 👤 **Скрытие неиспользуемых медиа при визите НО**: Скрытие неиспользуемых медиа при визитах районного надзирателя для поддержания чистоты презентаций.
- 🎵 **Улучшен индикатор дубликатов песен**: Улучшен визуальный сигнал для дубликатов песен для облегчения их идентификации.

## 25.4.3

### 🛠️ Улучшения и исправления

- ➕ **Очистка медиа из версий v25.4.x**: Автоматическая очистка диска от поврежденных медиа из версий v25.4.1-25.4.2.

## 25.4.2

### 🛠️ Улучшения и исправления

- ➕ **Предотвращение дублирования медиа**: Избегайте добавления некоторых медиа-элементов несколько раз в список.

## 25.4.1

### 🛠️ Улучшения и исправления

- 🎬 **Исправление пользовательского времени**: Предотвращение некорректного применения времени начала и окончания.
- 📝 **Разрешение несовпадающих субтитров**: Разрешено использование субтитров, даже если они не совсем совпадают с медиа-файлом.
- 🪟 **Отключение закругленных углов в Windows**: Удалены закругленные углы окна мультимедиа в Windows.
- 🖼️ **Включение нессылаемых изображений**: Убедитесь, что все изображения, на которые нет ссылок, добавлены в список.
- ➕ **Предотвращение дублирования разделов**: Избегайте создания нескольких разделов для одного и того же элемента.
- 📥 **Сохранение порядка плейлистов при импорте**: Поддержание исходного порядка плейлистов JWL во время импорта.
...
- 🎬 **Несколько временных меток для одного видео**: Разрешено многократное появление одного видео в списке с разными временами начала и окончания.
- 📤 **Включение сгруппированных медиа в автоэкспорт**: Экспорт сгруппированных элементов вместе с другими.
- 📡 **Правильное извлечение `.m4v` из JW API**: Убедитесь, что файлы `.m4v` корректно загружаются.
...
- ⚡ **Улучшение производительности и использования CPU**: Оптимизация для снижения нагрузки на процессор.
- 🔄 **Исправлены проблемы синхронизации и сбоев**: Решены различные проблемы со стабильностью.
- 📜 **Примечания к выпуску для существующих собраний**: Обеспечено отображение примечаний к выпуску для уже загруженных собраний.
...
- ⚡ **Оптимизация умной очистки кэша**: Улучшен механизм очистки кэша.
- 📂 **Правильное размещение медиа для НО**: Убедитесь, что медиа районного надзирателя находятся в правильном разделе.
- 📅 **Исключение обычных медиа во время Вечери**: Предотвращение загрузки стандартных медиа для встреч во время Вечери.
- 📅 **Скрытие разделов обычных встреч во время Вечери**: Удаление ненужных разделов для чистоты интерфейса.
- 📖 **Исправлена загрузка видео для жестового языка**: Корректная загрузка видео из плейлистов с главами Библии на жестовом языке.
