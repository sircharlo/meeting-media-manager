...
- ✨ **Automatisk synkronisering av mötesschema**: Möjlighet har lagts till att automatiskt synkronisera mötesdatum och tider med den officiella webbplatsen. Den här funktionen är aktiverad som standard och kan utlösas manuellt eller inaktiveras i de avancerade inställningarna.
- ✨ **Framtida schemaändringar**: Appen inkluderar nu framtida schemaändringar vid skapande av en församling via webbplatssökningen, om sådana finns tillgängliga.
- ✨ **Delad cache för maskinomfattande installationer**: Maskinomfattande installationer delar nu som standard en gemensam datamapp, vilket optimerar lagrings- och bandbreddsanvändningen mellan flera användare på samma dator.
