...
- 🐞 **Dynamic Media**: A dynamic media item that's no longer present in a fresh fetch (e.g. removed by JW.org) is now actually removed from the schedule instead of lingering indefinitely, unless it's the item currently playing.
