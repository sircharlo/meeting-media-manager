<template>
  <v-app>
    <v-navigation-drawer v-model="drawer" :mini-variant.sync="mini" permanent app>
      <v-list-item>
        <v-list-item-content>
          <v-list-item-title class="text-h6">M³</v-list-item-title>
          <v-list-item-subtitle>Meeting Media Manager</v-list-item-subtitle>
        </v-list-item-content>
        <v-btn v-if="!mini" color="indigo" @click.stop="mini = !mini">
      ...
      <v-divider />
      <v-list dense nav>
        <v-list-item v-for="(item, index) in navigationItems" :key="index" :to="item.to">
...
export default defineComponent({
  ...
  data() {
    return {
      ...
      mini: true,
      navigationItems: [
        { id: 1, name: 'Media calendar', icon: faCalendarDays, to: "/" },
        { id: 2, name: 'Media playback', icon: faChromecast, to: "/present" },
        { id: 3, name: 'Settings', icon: faGear, to: "/settings" }
      ]
    }
  },
  head() {
  ...
  async mounted() {
    ...
    const cong = this.$route.query.cong
    if (!cong) {
      this.$router.push({
        path: this.localePath('/congregation'),
  ...
  methods: {
    ...
    //         url: `${this.$config.repo}/discussions/new?category=translations&title=New+translation+in+${mediaLang.name}&language=I+would+like+to+help+translate+M³+into+a+language+I+speak,+${mediaLang.name} (${mediaLang.langcode}/${mediaLang.symbol}).`,
    //       },
    //     })
    ...
    //         type: 'link',
    //         label: 'wannaHelpForSure',
    //         url: `${this.$config.repo}/discussions/new?category=translations&title=New+translator+for+${appLang.name}&language=I+would+like+to+help+translate+M³+into+a+language+I+speak,+${appLang.name} (${appLang.langcode}/${appLang.symbol}).`,
    //       },
    //     })
    //   }
