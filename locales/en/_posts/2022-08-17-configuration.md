...
| `Custom cache path` | By default publications and other cache is saved in the user specific application data directory. You can change it if you want to share the cache between multiple users on the same computer. |
| `Date format for meeting folders` | The date format used for the meeting folders. <br><br> ***Note:** When using a congregation server, make sure everyone uses the same date format.* |
...
| `OBS Studio scene to display Zoom participants` | An optional scene to quickly and efficiently manage the display of Zoom participants during a hybrid meeting. <br><br> When this scene is configured, a toggle button will appear in the [Media Presentation mode]({{page.lang}}/#present-media). When the toggle button is activated, the media window and obs scene picker will be hidden and the configured Zoom scene will be used. After sharing media, the media window will disappear immediately. When the toggle is disabled, the media window will be shown again. |
| `Disable automatic app update` | When this option is enabled, M³ will not automatically update when closed. |
...
| `Fallback media language` | This language is used whenever the primary media language is not available. For example, if you select Irish as your media language and English as fallback. Whenever a publication or video is not available in Irish, it will be fetched in English. |
...
| `Enable subtitles for videos` | Enable this option if you want subtitles for videos whenever available. Subtitles will automatically be shown, but can be toggled while presenting. |
...
| `Hide media window after media finished playing` | If enabled, the media window will be hidden immediately after each media has finished playing. <br> <br><br> ***Note:** This setting is mostly useful for meetings held in sign-language. |
...
| `Include printed media when available` | If enabled, images of the printed versions of publications will be included when available. This could be useful for some tables or groups of images that are clearer in printed form. |
...
| `Enable button to play Kingdom songs on shuffle` | Enable a button on the main screen which will play Kingdom songs from the *sjjm* series, in random order. This is useful, for example, to play songs before and after meetings at the Kingdom Hall as background music. To the right of this option is a button to download all available Kingdom songs beforehand. |
