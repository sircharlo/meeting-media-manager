...
| `Next planned visit of the Circuit Overseer` | When you select a date for the next planned visit of the Circuit Overseer, M³ will automatically set the midweek meeting day for that week to Tuesday, skip the Congregation Bible Study media and the last Kingdom Songs for both meetings. <br><br> ***Note:** When using a congregation server, make sure to lock this setting so that all connected brothers have the same Circuit Overseer date configured.* |
