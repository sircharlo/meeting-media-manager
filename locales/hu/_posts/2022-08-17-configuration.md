...
| `Téma` | Select the theme you prefer. If you select `System`, M³ will use the system's theme. |
| `Gyülekezet név` | The name of your congregation. This is used to support multiple congregations who share the same computer. |
...
| `Dátumformátum az összejövetel mappáihoz` | The date format used for the meeting folders. <br><br> ***Note:** When using a congregation server, please ensure that everyone configures the same date format.* |
...
| `A feliratok bekapcsolása a médiához` | Enable this option if you want to fetch subtitles for videos, whenever available. Subtitles will be shown by default, but can be toggled on/off while presenting. |
