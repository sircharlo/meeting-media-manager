...
- **macOS**: macOS 10.15 (Catalina) och senare (universell version)
...
### :computer: Hur laddar jag ned och installerar M³? {#installation}

Ladda ner lämplig version från [Nedladdningssidan](download) och följ stegen i [Användarhandboken](user-guide).

### :gear: Hur ställer jag in M³ för första gången? {#first-time-setup}

M³ innehåller en installationsguide som guidar dig genom installationen:
...
### :desktop_computer: Hur laddar M³ ner media? {#media-download}
...
### :open_file_folder: Hur importerar jag mina egna mediefiler? {#import-media}
...
### :notes: Hur fungerar bakgrundsmusiken? {#faq-background-music}
...
### :computer: Vilka hårdvaru- och mjukvarukrav har M³? {#hardware-and-software-requirements}

M³ är utformad för att fungera på ett stort urval av system:

- **Windows**: Windows 10 och senare (64-bitars och 32-bitars versioner tillgängliga)
- **macOS**: macOS 10.15 (Catalina) och senare (universell version)
- **Linux**: De flesta moderna Linuxdistributioner (AppImage format)

M³ har följande hårdvarukrav:
...
Beroende på vilka funktioner du använder, kräver M³ också följande ytterligare programvara:

- **Zoom**: Krävs endast om du använder Zoom integrationsfunktioner
- **OBS Studio**: Behövs endast om OBS integrationsfunktioner används
